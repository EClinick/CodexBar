...
struct CodexResetCreditAutomationTests {
    ...
    @Test
    func `mock App Server drives real snapshot mapping and consume RPC`() async throws {
        let alertFixture = try CodexResetCreditAutomationDemo.makeFixture(
        ...
        defer { alertFixture.cleanup() }
        let alertAccount = try await alertFixture.fetcher.loadLatestCLIAccountSnapshot()
        let alertUsage = try #require(alertAccount.usage)
        let alertSnapshot = try #require(alertUsage.codexResetCredits)
        let alertCredit = try #require(alertSnapshot.credits.first)
        ...
        #expect(alertUsage.accountEmail(for: .codex) == CodexResetCreditAutomationDemo.accountEmail)
        #expect(alertCredit.id == alertFixture.stableCreditID)
        #expect(alertPlan.expiryAlerts.count == 1)
        let alertFireDate = try #require(alertPlan.expiryAlerts.first?.fireDate)
        #expect(alertFireDate.timeIntervalSince(planningNow) >= 1)
        #expect(alertFireDate.timeIntervalSince(planningNow) < 4)
        #expect(alertPlan.autoRedemptions.isEmpty)

        let autoFixture = try CodexResetCreditAutomationDemo.makeFixture(
            ...
            expiresAt: Date().addingTimeInterval(
                CodexResetCreditAutomationPlan.autoRedeemLeadTime +
                    CodexResetCreditAutomationDemo.phaseDelay))
        defer { autoFixture.cleanup() }
        let autoAccount = try await autoFixture.fetcher.loadLatestCLIAccountSnapshot()
        let autoUsage = try #require(autoAccount.usage)
        let autoSnapshot = try #require(autoUsage.codexResetCredits)
        let autoCredit = try #require(autoSnapshot.credits.first)
        let idempotencyKey = try #require(UUID(uuidString: "11111111-2222-3333-4444-555555555555"))
        let outcome = try await autoFixture.fetcher.consumeRateLimitResetCredit(
            creditID: autoCredit.id,
            idempotencyKey: idempotencyKey,
            expectedAccountEmail: CodexResetCreditAutomationDemo.accountEmail)
        let record = try await autoFixture.waitForConsumeRecord()

        #expect(outcome == .reset)
        #expect(autoCredit.id == autoFixture.stableCreditID)
        #expect(record == .init(
            creditId: CodexResetCreditAutomationDemo.autoRedeemProviderCreditID,
            idempotencyKey: idempotencyKey.uuidString))
    }

    @Test
    func `isolated fetcher never falls back when its exact executable disappears`() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("codexbar-isolated-fetcher-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        let isolatedExecutableURL = root.appendingPathComponent("isolated-codex")
        let fallbackExecutableURL = root.appendingPathComponent("fallback-codex")
        let fallbackMarkerURL = root.appendingPathComponent("fallback-ran")
        try Data("#!/bin/sh\nexit 0\n".utf8).write(to: isolatedExecutableURL)
        try Data("#!/bin/sh\nprintf ran > '\(fallbackMarkerURL.path)'\nexit 91\n".utf8)
            .write(to: fallbackExecutableURL)
        try FileManager.default.setAttributes(
            [.posixPermissions: 0o755],
            ofItemAtPath: isolatedExecutableURL.path)
        try FileManager.default.setAttributes(
            [.posixPermissions: 0o755],
            ofItemAtPath: fallbackExecutableURL.path)
        let fetcher = try UsageFetcher(
            isolatedCodexExecutableURL: isolatedExecutableURL,
            environment: ["CODEX_CLI_PATH": fallbackExecutableURL.path])
        try FileManager.default.removeItem(at: isolatedExecutableURL)

        do {
            _ = try await fetcher.loadLatestCLIAccountSnapshot()
            Issue.record("Expected the isolated fetcher to reject its missing exact executable")
        } catch {
            // Expected: the pinned resolver fails closed without launching the fallback.
        }
        #expect(FileManager.default.fileExists(atPath: fallbackMarkerURL.path) == false)
    }

    @Test
    func `isolated end to end test completes both production controller paths`() async {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("codexbar-reset-demo-store-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let environment = [
            "HOME": root.path,
        ...
        await store.runCodexResetCreditAutomationDemo()

        guard case let .succeeded(message) = store.codexResetCreditAutomationDemoState else {
            Issue.record("Expected the isolated reset automation test to succeed")
            return
        }
        #expect(message.contains(CodexResetCreditAutomationDemo.autoRedeemProviderCreditID))
        #expect(message.contains("idempotency key"))
        #expect(settings.codexResetExpiryNotificationsEnabled == false)
        #expect(settings.codexResetAutoRedeemEnabled == false)
    }
